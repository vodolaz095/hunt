...
//finders
  ...
  UserSchema.statics.findOneByEmail = function (email, callback) {
    this.findOne({'$or':[{'keychain.email': email},{'keychain.username': email}]}, callback);
  };
  /**
...
//groups
  ...
  /**
   * @method User#createMyGroup
   * @param {string} groupName
   * @param {function} callback
   * @description
   * Create group with user as owner
   */
  UserSchema.methods.createMyGroup = function (groupName, callback) {
    var thisUser = this;
    core.model.Group.create(
      { 'name': groupName, 'owner': thisUser._id }, {'upsert': false}, function (err, group) {
        if (group) {
          core.model.User.findOneAndUpdate({'_id': thisUser._id},
            { $addToSet: { 'groups': group._id } },
            function (err, userUpdated) {
