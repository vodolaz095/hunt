      ...
      User.findOneByKeychain(profile.provider, profile.id, function (err, userFound) {
        if (err) {
          ...
          if (userFound) {
            ...
            winston.info('user:signin:%s:%s', profile.provider, userFound.id, {
              'ip': request.ip,
              'ips': request.ips,
              'userAgent': request.headers['user-agent'],
              'userId': userFound.id,
            ...
            var keychain = {};
            keychain[profile.provider] = profile.id;
            User.create({
              ...
              if (err) {
                ...
                core.emit(['user', 'signup', profile.provider], {
                  'ip': request.ip,
                  'ips': request.ips,
                  'userAgent': request.headers['user-agent'],
                  'method': profile.provider,
                ...
                winston.info('user:signup:%s:%s', profile.provider, userCreated.id, {
                  'ip': request.ip,
                  'ips': request.ips,
                  'userAgent': request.headers['user-agent'],
...
//dialog messages system
  ...
  UserSchema.statics.listFilter = function (user, callback) {
    if (user) {
      if (user.root) {
        return callback(null, {}, fieldsReadableByRootOrSelf);
      } else {
        return callback(null, {'accountVerified': true}, fieldsReadable);
      }
    }
    return callback(null, false);
  };
  ...
  UserSchema.methods.canRead = function (user, callback) {
    ...
    return callback(null, true, fieldsReadable);
  };

  UserSchema.methods.canUpdate = function (user, callback) {
    var t = this;
    if (user && t._id.equals(user._id)) {
      return callback(null, true, fieldsWritable);
    }
    if (user && user.root) {
      return callback(null, true, fieldsWritableByRoot);
    }
    return callback(null, false);
  };
  ...
  var User = core.mongoConnection.model('User', UserSchema);
  User.type = 'mongoose';
  return User;
};
