...
//dialog messages system
  ...
  UserSchema.methods.canRead = function (user, callback) {
    if (user.id == this.id || user.root) {
      callback(null, true, ['id', 'accountVerified', 'keychain', 'lastSeenOnline', 'name', 'displayName', 'roles', 'root', 'isBanned', 'profile']);
    } else {
      callback(null, false);
    }
  };
  ...
  UserSchema.methods.canUpdate = function (user, callback) {
    if (user && user.id == this.id) {
      callback(null, true, ['name', 'displayName', 'profile', 'password']);
      return;
    }
    if (user && user.root) {
      callback(null, true, ['name', 'displayName', 'roles', 'root', 'isBanned', 'profile', 'password']);
      return;
    }
    callback(null, false);
  };
