...
//      'profile': this.profile,
  ...
  UserSchema.methods.toString = function () {
    if(this.name && this.name.familyName){
      return this.name.givenName +' '+ this.name.middleName +' '+ this.name.familyName;
    }

    return this.displayName || this.username || ('User #' + this.id);
  };
...
//local strategy (username/email and password)
  ...
  UserSchema.statics.signUp = function (email, password, callback) {
    return Q.ninvoke(this, 'create', {
      ...
      'keychain': {'email': email.toString().toLowerCase(), 'welcomeLink': core.rack()},
      'password': password
    })
