...
module.exports = exports = function (core) {
  ...
  messageSchema.statics.listFilter = function (user, callback) {
    if (user) {
      if (user.root) {
        ...
        callback(null, {$and: [{$or: [{'to': user._id}, {'from': user._id}]}]});
      }
    } else {
      callback(null, false);
    }
  ...
  messageSchema.methods.canRead = function (user, callback) {
    if (user && ((user._id.equals(this.to._id || this.to) || user._id.equals(this.from._id || this.from) || user.root))) {
  ...
  messageSchema.methods.toString = function () {
    return util.format('Message #%s', this.id);
  };
  ...
  var Message = core.mongoConnection.model('Message', messageSchema);
  Message.type = 'mongoose';
  return Message;
};
