...
//sending user email for account verification
                if (core.config.passport.verifyEmail) {
                  ...
                  user.notifyByEmail({
                    ...
                    'verifyUrl': core.config.hostUrl + 'auth/confirm/' + user.huntKey
                  });
                } else {
          ...
          if (userFound) {
            userFound.invalidateSession(function (err) {
              ...
              userFound.notifyByEmail({
                ...
                'resetUrl': core.config.hostUrl + 'auth/reset/' + userFound.huntKey
              });
            });
          }
...
//route to render page with form to reset password
    core.app.get('/auth/reset/:huntKey', function (request, response) {
      if (request.user) {
        response.redirect('/');
      } else {
        request.model.User.findOneByHuntKey(request.params.huntKey, function (err, userFound) {
          if (err) {
            throw err;
          } else {
            if (userFound) {
              console.log(userFound.huntKey);
              response.render('' + core.config.passport.resetPasswordPageTemplate, {
                'title': 'Reset your password',
                'description': 'Welcome back!',
                'huntKeyForReset': userFound.huntKey
              });
            } else {
              response.send(404);
            }
...
//route to reset password
    core.app.post('/auth/resetPassword', function (request, response) {
      if (request.body.huntKey && request.body.password) {
        request.model.User.findOneByHuntKeyAndResetPassword(request.body.huntKey, request.body.password, function (err) {
          if (err) {
