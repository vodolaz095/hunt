 ...
 * @name Hunt#express
 * @description
 * {@link http://expressjs.com/ | ExpressJS} framework constructor
 */
...
//working with dialog and group chat events
...
//start configuring middlewares
  core.app.enable('trust proxy'); //because I DO insist of running Hunt behind nginx

  core.app.use(function (req, res, next) {
    if (toobusy()) {
      res.send(503, 'Server is overloaded. Reload the page. Sorry...');
    } else {
      next();
    }
  });
...
//setting up the view engine
  switch (core.config.templateEngine) {
    ...
    case 'swig':
      ...
      break;
    case 'jade':
      require('./templateEngines/jadeEngine.js')(core); //todo implement
      break;
    case 'ejs':
