...
module.exports = exports = function (core) {
  var transport,
    ...
    defaultCallback = function (error, response) {
      if (error) {
        ...
        winston.log('verbose', 'Sending email was successful!', response);
      }
    },
    fromEmailAddr;
  ...
  if (config && typeof config === 'object') {
    ...
    if (config.service) {
      if (!(config.auth && config.auth.user && config.auth.pass)) {
        winston.warn('Mail Dispatcher: unable to find credentials! using fallback Direct transport. This is not recommended for production use!');
        transport = nodemailer.createTransport();
      } else {
        winston.silly('Mail Dispatcher: using options from hunt.config.emailConfig for service "%s"', config.service);
        transport = nodemailer.createTransport(config);
      }
    } else {
      winston.silly('Mail Dispatcher: using options from hunt.config.emailConfig...');
      transport = nodemailer.createTransport(config);
    }
    fromEmailAddr = core.config.emailConfig.fromEmailAddr;
  } else {
    winston.warn('Mail Dispatcher: using fallback Direct transport. This is not recommended for production use!');
    var hostname = url.parse(core.config.hostUrl).hostname;
    transport = nodemailer.createTransport(directTransport({
      name: hostname,
      logger:winston
    }));
    fromEmailAddr = 'hunt@' + hostname;
  }
  ...
  core.on('user:notify:email', function (messageObj) {
    if (core.config.env === 'development') {
      winston.info('Sending email to %s', messageObj.user.email, messageObj.message);
    }
