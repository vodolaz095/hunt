...
module.exports = exports = function (core, parameters, router) {
  router.delete(/^\/([0-9a-fA-F]{24}||\d+)$/, core.preload(parameters.modelName), function (request, response) {
    if (request.preload.canDelete === true) {

      int.delete(request.model[request.preload.modelName], request.preload.model, function (error) {
        if (error) {
          throw error;
        }
        /**
