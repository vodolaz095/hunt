...
module.exports = exports = function (configToUse) {
  ...
  config.port = config.port || parseInt(process.env.PORT) || 3000;
...
// see for details - https://github.com/Automattic/engine.io#methods-1
  ...
  config.io = config.io || {};
  config.io.pingTimeout = config.io.pingTimeout || 5000;
  config.io.pingInterval = config.io.pingInterval || 10000;
  config.io.allowUpgrades = config.io.allowUpgrades !== undefined  ? config.io.allowUpgrades : true;
  config.io.transports = config.io.transports || ['polling', 'websocket'];
