...
module.exports = exports = function (core) {
  ...
  if (core.config.redisUrl && core.config.redisUrl.port && core.config.redisUrl.host) {
    ...
    redisConfigUrlParsed = url.parse(core.config.redisUrl);
    if (redisConfigUrlParsed && redisConfigUrlParsed.protocol === 'redis:') {
      config = {
...
//      console.log('redis.send_command',command, args, args[args.length-1],callback);
      ...
      return original.apply(this, constructedArguments);
    };



    r.on('ready', function(){
      winston.debug('Process %s established connection to redis://%s:%s',
        process.pid, config.host, config.port
      );
    });

    r.on('end', function(){
      winston.debug('Process %s closed connection to redis://%s:%s',
        process.pid, config.host, config.port
      );
    });
