  ...
  if (core.config.mongoUrl) {
    var
      mongoConnection = mongoose.createConnection(core.config.mongoUrl, {
        'server' : { 'poolSize' : 3, 'socketOptions' : { keepAlive : 1 } }
      }),
    ...
    MongoCollectionOperations.map(function (x) {
      ...
      //console.log('===MongoProfiling====', x, arguments, arguments.length, '===MongoProfiling====');
        var
          i,
          args = [],
          callback = arguments[arguments.length - 1],
          startedAt = new Date(),
          databaseName = this.s.dbName, // https://github.com/mongodb/node-mongodb-native/blob/2.0/lib/collection.js#L99
