...
var
  ...
  dialogRouter,
  dialogController = require('./dialog.controller.js');
...
//injecting models and other usefull into request
    ...
    request.huntEmit = function (eventName, eventPayload) {
      console.log('huntEmit', eventName, eventPayload);
      return core.emit(eventName, eventPayload);
    };
    request.io = core.io;
...
//adding dialog endpoints
  if (core.config.dialog) {
    ...
    core.exportModelToRest({
      'mountPount': '/api/v1/messages',
      'modelName': 'Message',
      'ownerId': 'from'
    });
    core.app.use('/api/v1/dialog', dialogRouter);
    core.app.use('/api/v1/dialog', dialogRouter);
  }
 ...
 *
 * @property {(Hunt#preload)|null} preload - data populated by {@link Hunt#preload} middleware
