...
var
  ...
  dialogRouter,
  dialogController = require('./messages.controller.js'),
  usersRouter,
...
// set shutdown procedure
   ...
   */
  core.extendCore('doHttpError', function() {
    return require('./errors.js').doError;
  });
...
// parse application/x-www-form-urlencoded
  core.app.use(bodyParser.urlencoded({
    'extended': false,
    'limit':core.config.bodyParserLimit || '100kb'
  }));
...
//different setup for different environments
  ...
  case 'production':
    winston.silly('Production environment!');
    ...
    break;
  case 'staging':
    winston.silly('Staging environment!');
    core.app.locals.environment = 'staging';
    core.app.locals.staging = true;
    core.app.use(require('response-time')());
    core.app.enable('view cache');
    winston.level=winston.level || 'silly';
    winston.cli();
    break;
  default:
    winston.silly('Development environment!');
    core.app.use(require('response-time')());
  ...
  core.app.use(function (request, response, next) {
    ...
    function emitHttpEvent() {
      ...
      var
        ...
        ver = request.httpVersionMajor + '.' + request.httpVersionMinor,
        responseLength = response.get('content-length'),
        httpSuccess = {
          'ip': request.ip,
          'ips': request.ips,
          'userAgent': ua,
          ...
          'contentLength': responseLength //todo - not works?
        };
      ...
      if (request.query) {
        httpSuccess.query = JSON.stringify(request.query);
      }
      if (request.body) {
        httpSuccess.body = JSON.stringify(request.body);
      }
      if (request.files) {
        httpSuccess.body = JSON.stringify(request.files);
      }
      if (request.user) {
        httpSuccess.userId = request.user.id;
        httpSuccess.user = request.user.toString();
      }

      winston.debug('http:success', httpSuccess);

      //prettify `http-success` events to make them more like Apache combined output
      var clfmonth = [
      ...
      function clfdate(dateTime) {
        var date = dateTime.getUTCDate(),
          ...
          month = clfmonth[dateTime.getUTCMonth()],
          mili = dateTime.getUTCMilliseconds();

        return pad2(date) + '/' + month + '/' + year + ':' + pad2(hour) + ':' + pad2(mins) + ':' + pad2(secs) + ' +' + mili;
      }

//:remote-addr - :remote-user [:date[clf]] ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent"
      winston.verbose('%s - %s%s "%s %s HTTP/%s" %d %d %s %s',
        (httpSuccess.ips ? httpSuccess.ips.join(' ') : httpSuccess.ip) || '127.0.0.1',
        httpSuccess.userId ? ('User#' + httpSuccess.userId + ' ') : ' ',
        clfdate(httpSuccess.startTime),
        httpSuccess.method,
        httpSuccess.uri,
        httpSuccess.httpVersion,
        httpSuccess.statusCode,
        httpSuccess.contentLength || 0, //todo - zero for firefox?
        httpSuccess.referer || '',
        httpSuccess.userAgent
      );
...
//adding dialog endpoints
  if (core.config.dialog) {
    ...
    dialogController(core, dialogRouter);
    core.app.use('/api/v1/message', dialogRouter);
    core.app.use('/api/v1/messages', dialogRouter);
  }
...
//error catcher for all other errors
      ...
      if (request.user) {
        httpError.userId = request.user.id;
        httpError.user = request.user.toString();
      }
      if (request.query) {
        httpError.query = JSON.stringify(request.query);
      }
      if (request.body) {
        httpError.body = JSON.stringify(request.body);
      }
      if (request.files) {
        httpError.body = JSON.stringify(request.files);
      }
      winston.error(httpError);
      next(error);
    }
  });
...
//mask passwords and huntKeys in `http:success` in event metadata
  winston.rewriters.push(function (level, message, meta) {
    if (core.config.env !== 'development') {
      if (message === 'http:success') {
        if (meta && meta.body && meta.body.password) {
          meta.body.password = '*******';
        }

        if (meta && meta.query && meta.query.huntKey) {
          meta.query.huntKey = '*******';
        }

        if (meta && meta.body && meta.body.huntKey) {
          meta.body.huntKey = '*******';
        }
      }
    }
