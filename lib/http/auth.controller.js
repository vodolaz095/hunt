...
//routes for signUp
      ...
      /*/
       ...
       response.redirect('/auth/success?strategy=signup');
       }
       });
       } else {
       if (request.is('json')) {
       ...
       response.redirect('/auth/failure?strategy=signup');
       }
       }
       });
       } else {
...
//route to ask for password reset email
      console.log('PassportManager:'.green + ' reset password enabled!');
...
//they can be overloaded in application by Hunt.extendRoutes(function(core){});
  ...
  router.all('/logout', function (req, res) {
    if (req.user) {
      req.flash('success', 'Goodbye, ' + req.user.toString() + '!');
      req.logout();
    }
    res.redirect('back');
  });
