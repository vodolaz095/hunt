...
module.exports = exports = function (extendAppFunctions, extendMiddlewareFunctions, extendRoutesFunctions) {
  var core = this;
 ...
 */
  core.app = express();
  core.app.disable('x-powered-by'); //sorry ;-)
/**
...
//setting default and always working settings for app
  ...
  core.app.use(require('body-parser')());
  if (core.config.uploadFiles) {
    throw new Error('not implemented yet!');
    //core.app.use(express.multipart()); //todo - change
  }
 ...
 * @property {string} ips - proxy chain if present - {@link http://expressjs.com/3x/api.html#req.ips}
 * @property {string} uri
 ...
 */
    var errorObject = {
      ...
      'startTime': request._startTime,
      'duration': (Date.now() - request._startTime),
//      'statusCode': response.statusCode, //it have to be 500, thats all
      'method': request.method,
      'query': request.query,
      'body': request.body,
      'ip': request.ip,
    ...
    core.emit('httpError', errorObject);
    next(error);
  });
